...
function Mine(){
	if(notReady())return;
	var table = newWindow.document.getElementsByClassName('main')[0].childNodes[0];
	getData(table);
	closeWindow();
	state = 0;
}
...
function getData(table){
	...
	for(var i=0;i<table.childNodes.length;i++){
		...
		}else if(row.getAttribute('class') == 'onem' || row.getAttribute('class') == 'twom' ){
			homeTeam = getHomeTeam(row);
			awayTeam = getAwayTeam(row);
			score = getScore(row);
				dataToPrint+=(dataToPrint.length>0?"\n":"")+league+','+homeTeam+','+awayTeam+','+score;
		}
    }
}
function findCountryLeague(row){
    var href = row.getElementsByTagName('a')[0].getAttribute('href');
	href = href.split('/');
